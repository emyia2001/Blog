...
- **内容加密（可选，三类通用）**：加 `lock: { question, answer }` 后，正文在构建时用 AES-256-GCM 加密成密文（**明文不进源码/产物**），网页上只显示「谜面 question」，输对 `answer` 才在浏览器本地解密显示。因明文不落产物、这是**真加密**；但密文+谜面仍在页面里，答案若过短可被离线破解，务必用长且独特的答案。解锁后以纯文本展示（不重建 MDX 排版）。
